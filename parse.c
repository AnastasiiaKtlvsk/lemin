...
t_r     *cr_r(t_l *tl, int t)
{
    ...
    r->f = (t == 0) ? tl->na : 0;
    r->st = 0;
    r->rl = ft_memalloc(sizeof(char) * 2048);
    r->pth = ft_memalloc(sizeof(char) * 2048);
    return (r);
}
...
int     pr_sw(t_l *tl, char *temp, int i, int fd)
{
    char    *line;
    t_r     *r;
    char    **pms;
...
{
  //  int fd = open("C:\\Users\\1\\CLionProjects\\lemin2\\test", O_RDONLY);
    if (!read_na(tl, temp, 0))
        return (0);
