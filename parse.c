...
int		pr_sw(t_l *tl, char *temp, int fd)
{
	...
	if (get_next_line(fd, &line) > 0)
	{
		...
		{
			r = (ft_strstr(temp, "start"))
			? cr_r(tl, 0, pms[0]) : cr_r(tl, 2, pms[0]);
...
int		cr_room(t_l *tl, char *temp)
{
	...
	if (pms && pms[0]
		&& ft_strlen(pms[0])
		&& pms[1] && ft_strlen(pms[1]) && pms[2] && !pms[3])
	{
		r = cr_r(tl, 1, pms[0]);
		tl->pr = ft_strcat(tl->pr, temp);
		tl->pr = ft_strcat(tl->pr, "\n");
...
int		rel2(char **rel, char **pms, t_r *tr1, t_r *tr2)
{
